...
from app.groups.schema import GroupSchema, GroupSchemaIn, GroupUserSchema
...
@group_router.get("/get-all-groups", response_model=list[dict])
def get_all_groups():
    return GroupController.get_all()


@group_router.get("/get-all-groups-with-members", response_model=dict)
def get_all_groups_with_members():
    return GroupUserController.get_all_joined()


@group_router.post("/create-group", response_model=GroupSchema, dependencies=[Depends(JWTBearer("classic_user"))])
...
@group_router.post("/join-group/{group_name}", response_model=GroupUserSchema,
...
@group_router.put("/change-group-name/", response_model=GroupSchema, dependencies=[Depends(JWTBearer("classic_user"))])
