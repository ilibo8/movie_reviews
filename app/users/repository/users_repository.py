...
class UserRepository:
    ...
    def create_user(self, user_name: str, password: str, email: str) -> User:
        ...
        if self.dbs.query(User).filter(User.user_name == user_name).first() is not None:
            raise AlreadyExist(f"User name {user_name} already exist.")
        if self.dbs.query(User).filter(User.email == email).first() is not None:
            raise AlreadyExist(f"Email {email} already exist.")
        self.dbs.add(user)
        self.dbs.commit()
        self.dbs.refresh(user)
        return user
    ...
    def get_user_by_id(self, user_id: int) -> Type[User] | None:
        ...
        """
        user = self.dbs.query(User).filter(User.id == user_id).first()
        return user
    ...
    def get_user_name_by_user_id(self, user_id: int) -> str:
        ...
        """
        user = self.dbs.query(User.user_name).filter(User.id == user_id).first()
        return user[0]
    ...
    def get_user_by_user_name(self, user_name: str) -> Type[User] | None:
        ...
        """
        user = self.dbs.query(User).filter(User.user_name == user_name).first()
        if user is None:
