...
@movie_router.get("/get-movies-by/actor-id/{actor_id}", response_model=list)
def get_movies_by_actor_id(actor_id: int):
    return MovieController.get_movies_by_actor_id(actor_id)
...
@movie_router.get("/get-all-movies-by/director", response_model=list[MovieSchema])
