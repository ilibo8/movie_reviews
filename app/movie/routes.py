...
from app.movie.schema import MovieSchema, MovieGenreSchema, MovieSchemaIn, MovieSchemaJoined

movie_router = APIRouter(prefix="/api/movies", tags=["Movies"])


@movie_router.get("/get-all-movies-by/word-in-title", response_model=list[MovieSchema])
...
@movie_router.post("/add-movie-genre", response_model=MovieSchemaJoined)
...
@movie_router.post("/add-movie-cast", response_model=MovieSchemaJoined)
def add_movie_cast(movie_id: int, actors_ids: list[int]):
    return MovieController.add_movie_cast(movie_id, actors_ids)
